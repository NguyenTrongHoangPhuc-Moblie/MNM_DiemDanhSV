...
function SuaChiTietNgayHoc() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/layCTNH/" + id)
      .then((res) => {
        setValues({
          ...values,
          MaNH: res.data.MaNH,
          TietHocBatDau: res.data.MaTH,
          TietHocKetThuc: res.data.MaTH,
          MaNH: res.data.MaNH,
        });
      })
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/api/suaCTNH", {
        Ngay: NgayHoc,
  ...
  async function xoaCTNH(id) {
    let result = fetch("http://localhost:8000/api/xoaCTNH/" + id, {
      method: "DELETE",
    });
  ...
  const changeTietHocBatDau = (event) => {
    setValues({ ...values, TietHocBatDau: event.target.value });
  };
  const changeTietHocKetThuc = (event) => {
    setValues({ ...values, TietHocKetThuc: event.target.value });
  };
          ...
          <Content>
            ...
            <form onSubmit={handleSubmit} className="col-sm-4 offset-sm-4">
              ...
              <div className="d-flex">
                ...
                <button onClick={() => xoaCTNH(NgayHoc)} className="btn btn-danger">Xóa</button>
              </div>
            </form>
          </Content>
        </Layout>
...
export default SuaChiTietNgayHoc;
