...
function HienThiNganh() {
  ...
  const truyXuat = async () => {
    let result = await fetch("http://localhost:8000/api/danhSachNganh");
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/danhSachNganh")
      .then((res) => {
        setData(res.data);
        setRecords(res.data);
      })
  ...
  async function xoaNganh(id) {
    let result = fetch("http://localhost:8000/api/xoaNganh/" + id, {
      method: "DELETE",
    });
    result = (await result).json();
    truyXuat();
                ...
                {records.map((item, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <Link>
                        <span
                          onClick={() => xoaNganh(item.MaNganh)}
                          className="btn btn-danger"
                        >
                          Xoá
                        </span>
                      </Link>
                      <Link to={`/suaNganh/${item.MaNganh}`}>
